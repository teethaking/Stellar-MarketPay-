...
// Audit logs are admin-only but still a privileged read surface: cap requests
// per IP so a compromised/leaked admin token cannot be used to enumerate the
// full audit trail (or to hammer the database with expensive filtered queries).
const auditReadRateLimiter = createRateLimiter(60, 1); // 60 requests/min per IP
